...
/// Location of a materialized free-file payload.  Mobile and GUI hosts use
/// this view to stream/copy large files without buffering them in memory.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct FileResourceLocationView {
    pub filename: String,
    pub mime: Option<String>,
    pub path: PathBuf,
    pub size_bytes: u64,
...
impl ResourceFacade {
    ...
    pub async fn entry_file(
        ...
        let location = self.entry_file_location(entry_id).await?;
        let bytes = tokio::fs::read(&location.path)
            .await
    ...
    /// Resolve an entry's first materialized free-file without reading its
    /// contents.  The returned path belongs to the daemon-managed cache.
    #[instrument(skip_all, fields(entry_id = %entry_id))]
    pub async fn entry_file_location(
        &self,
        entry_id: &str,
    ) -> Result<FileResourceLocationView, ResourceFacadeError> {
        ...
        let metadata = tokio::fs::metadata(&path)
            .await
            .map_err(|err| ResourceFacadeError::Internal(format!("failed to inspect file: {err}")))?;

        Ok(FileResourceLocationView {
            filename,
            mime,
            path,

...
pub fn detect_storage_capability() -> SecureStorageCapability {
    // OpenHarmony applications do not provide a freedesktop Secret Service.
    // Keep all key material in the application-private data directory; the
    // file backend encrypts the persisted business payload with the MasterKey.
    #[cfg(target_env = "ohos")]
    {
        tracing::info!("OpenHarmony detected. Using app-private file secure storage.");
...
pub fn detect_system_clipboard_capability() -> SystemClipboardCapability {
    // ArkUI owns pasteboard access. The Rust space node is driven through the
    // N-API bridge, so desktop X11/Wayland clipboard discovery must stay off.
    #[cfg(target_env = "ohos")]
    {
        return SystemClipboardCapability::NoDisplaySession;
    }

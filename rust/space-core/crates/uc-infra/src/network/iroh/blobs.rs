...
impl BlobTransferPort for IrohBlobTransferAdapter {
    ...
    async fn shutdown_inflight_fetch(&self, ticket: &BlobTicket) -> Result<(), BlobError> {
        // Upstream iroh-blobs 0.102 no longer exposes the old vendor-patched
        // `Downloader::shutdown_endpoint` hook. Closing the shared iroh
        // Endpoint here would also tear down pairing, clipboard and every
        // concurrent transfer, so do not fake a successful cancellation.
        // Keep the limitation observable until cancellation is represented by
        // a first-class per-download handle in the adapter.
        let native = Self::parse_ticket(ticket)?;
        ...
        warn!(
            ...
            "blob fetch: selective cancellation is unavailable with iroh-blobs 0.102"
        );
        Err(BlobError::Unavailable(
            "selective blob download cancellation is unavailable".into(),
        ))
    }

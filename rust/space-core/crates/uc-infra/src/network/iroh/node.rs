...
use uc_core::ports::blob::{BlobDigest, BlobError, BlobProgressSink, BlobTicket, TagReason};
...
/// Phase-one OpenHarmony fallback while the patched `iroh-blobs` submodule is
/// unavailable in source archives. Small inline clipboard payloads continue
/// to use the regular clipboard ALPN; blob-backed payloads report a precise
/// capability error instead of preventing the space node from starting.
#[cfg(not(feature = "blobs"))]
struct DisabledBlobTransfer;

#[cfg(not(feature = "blobs"))]
impl DisabledBlobTransfer {
    fn unavailable() -> BlobError {
        BlobError::Unavailable("blob transfer is not enabled in this OpenHarmony build".into())
    }
}

#[cfg(not(feature = "blobs"))]
#[async_trait::async_trait]
impl BlobTransferPort for DisabledBlobTransfer {
    async fn publish(
        &self,
        _ciphertext: bytes::Bytes,
        _reason: TagReason,
    ...
    fn digest_of(&self, _ticket: &BlobTicket) -> Result<BlobDigest, BlobError> {
        Err(BlobError::InvalidTicket)
    }
}
...
impl IrohNodeBuilder {
    ...
    pub async fn install_blobs(
        ...
        info!("iroh blob transport disabled; inline clipboard sync remains available");
        Ok(BlobHandlers {
            blob_transfer: Arc::new(DisabledBlobTransfer),
        })
    }

...
/// Process-local paths supplied by the HarmonyOS UIAbility before the native
/// space node starts. They point at the application's private sandbox.
pub const OHOS_DATA_DIR_ENV: &str = "UC_OHOS_DATA_DIR";
pub const OHOS_CACHE_DIR_ENV: &str = "UC_OHOS_CACHE_DIR";
...
pub fn base_data_local_dir() -> Option<PathBuf> {
    std::env::var_os(OHOS_DATA_DIR_ENV).map(PathBuf::from)
}
...
pub fn base_cache_dir() -> Option<PathBuf> {
    std::env::var_os(OHOS_CACHE_DIR_ENV)
        .or_else(|| std::env::var_os(OHOS_DATA_DIR_ENV))
        .map(PathBuf::from)
}
...
fn platform_log_dir(app_dir_name: &str) -> Option<PathBuf> {
    Some(
        PathBuf::from(std::env::var_os(OHOS_DATA_DIR_ENV)?)
            .join(app_dir_name)
            .join("logs"),
    )
}

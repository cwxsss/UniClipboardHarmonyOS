...
use uc_mobile_proto::{
    ...
    HistoryRecord as ProtoHistoryRecord, HistoryRecordPatch as ProtoHistoryRecordPatch,
};
...
/// Mutable fields accepted by `PATCH /api/history/<type>/<hash>`.
#[derive(Debug, Clone, Default, PartialEq, Eq, uniffi::Record)]
...
impl HistoryPatch {
    fn into_proto(self) -> ProtoHistoryRecordPatch {
        ProtoHistoryRecordPatch {
            starred: self.starred,
            pinned: self.pinned,
...
impl MobileSyncClient {
    ...
    /// `PATCH /api/history/<type>/<hash>` — update favorite/pin/delete flags.
    /// The protocol deliberately uses the split path form and `isDelete`
    /// JSON field; both shapes are produced here instead of by platform code.
    pub async fn patch_history(
        &self,
        server: ServerConfig,
        kind: ClipboardKind,
        hash: String,
        patch: HistoryPatch,
    ) -> Result<HistoryRecord, SyncError> {
        validate_path_component(&hash, "hash")?;
        let body = patch
            .into_proto()
            .to_json()
            .map_err(|error| SyncError::Internal {
                reason: format!("encode history patch: {error}"),
            })?;
        let http = self.http();
        self.run(async move {
            let url = endpoint(
                &server.base_url,
                &["api", "history", kind.wire_str(), &hash],
            )?;
            let req = http
                .patch(url)
                .basic_auth(&server.username, Some(&server.password))
                ...
                .body(body);
            let resp = check(send_with_retry(req).await?).await?;
            let record: ProtoHistoryRecord = resp
                .json()
                .await
                .map_err(decoding("history patch"))?;
            Ok(HistoryRecord::from(record))
        })
        .await
    }
...
pub(crate) mod tests {
    ...
    async fn mock_history_patch(
        ...
        let body = String::from_utf8(body.to_vec()).expect("history patch utf8");
        state.record(format!("history-patch:{kind}:{hash}:{body}"));
        let record = state
            .cfg
            .history
            .first()
            .cloned()
            .unwrap_or_else(|| ProtoHistoryRecord::new(hash, ProtoKind::Text));
    ...
    pub(crate) async fn spawn_mock(cfg: MockConfig) -> (SocketAddr, Arc<MockState>) {
        ...
        let app = Router::new()
            ...
            .route("/api/history/:kind/:hash", patch(mock_history_patch))
    ...
    #[tokio::test]
    async fn patch_history_uses_split_path_and_protocol_json() {
        let mut response = ProtoHistoryRecord::new("ABC123", ProtoKind::Text);
        response.text = Some("updated".into());
        response.starred = true;
        response.version = Some(4);
        let (addr, state) = spawn_mock(MockConfig {
        ...
        assert_eq!(record.text.as_deref(), Some("updated"));
        assert!(record.starred);
        assert_eq!(record.version, Some(4));
        assert_eq!(
            state.events(),
            vec![
                "history-patch:Text:ABC123:{\"starred\":true,\"isDelete\":false,\"version\":3}"
            ]
        );
    }
